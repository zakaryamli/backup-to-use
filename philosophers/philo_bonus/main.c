...
/*   Updated: 2024/06/10 19:39:24 by zyamli           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	philo_init(t_table *table)
{
	...
	table->fork_lock = sem_open("/forks_lock",
			O_CREAT, 0777, table->philos_num);
	...
	if (table->end_simu == SEM_FAILED)
		return (print_error("sem error"));
	table->pid = zyalloc(sizeof(int) * table->philos_num, 'a');
	if (!table->pid)
		return (print_error("malloc erorr"));
	philo_stuff(table);
}
